...
<template>
  ...
  <nav class="fixed bottom-0 left-0 right-0 bg-amber-50 text-amber-900 border-t-2 border-amber-200 shadow-sm z-20">
    <div class="flex justify-around items-center h-16">
      <!-- Home Button -->
      <button @click="navigateTo('home')" :class="['flex flex-col items-center justify-center w-1/4 h-full transition-colors',
        activeTab === 'home' ? 'bg-amber-100 text-green-700' : 'hover:bg-amber-100']">
      ...
      <!-- Machines Button -->
      <button @click="navigateTo('machines')" :class="['flex flex-col items-center justify-center w-1/4 h-full transition-colors',
        activeTab === 'machines' ? 'bg-amber-100 text-green-700' : 'hover:bg-amber-100']">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
      ...
      <!-- Seasons Button -->
      <button @click="navigateTo('seasons')" :class="['flex flex-col items-center justify-center w-1/4 h-full transition-colors',
        activeTab === 'seasons' ? 'bg-amber-100 text-green-700' : 'hover:bg-amber-100']">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
      ...
      <button @click="showSettingsDialog = true"
        class="flex flex-col items-center justify-center w-1/4 h-full hover:bg-amber-100 transition-colors">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
  ...
  <div v-if="showSettingsDialog" class="fixed inset-0 bg-black bg-opacity-50 z-30" @click="showSettingsDialog = false">
    <div
      class="absolute bottom-0 left-0 right-0 bg-amber-50 rounded-t-xl p-4 transform transition-transform duration-300"
      @click.stop>
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold text-amber-900">Settings</h2>
        <button @click="showSettingsDialog = false" class="text-amber-700 hover:text-amber-900">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="mb-6">
        ...
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            <span class="text-amber-900">Auto-Save:</span>
            <button @click="persistenceStore.toggleAutoSave"
              :class="persistenceStore.autoSaveEnabled ? 'bg-green-600' : 'bg-amber-300'"
          ...
          <div class="flex items-center justify-between">
            <span class="text-amber-900">Offline Progress:</span>
            <button @click="persistenceStore.toggleOfflineProgress"
              :class="persistenceStore.offlineProgressEnabled ? 'bg-green-600' : 'bg-amber-300'"
              class="relative inline-flex items-center h-6 rounded-full w-11 transition-colors focus:outline-none">
        ...
        <div class="space-y-2">
          ...
          <div class="flex space-x-2 mt-2">
            ...
            <button @click="persistenceStore.loadGame"
              class="flex-1 bg-amber-400 hover:bg-amber-500 text-amber-900 py-2 px-4 rounded-md transition-colors">
              Load Game
            </button>
          </div>
        </div>
      ...
      <div>
        ...
        <button @click="confirmResetSave"
          class="w-full bg-red-100 hover:bg-red-200 text-red-600 border border-red-300 py-2 px-4 rounded-md transition-colors">
          Reset All Progress
        </button>
      </div>
    </div>
