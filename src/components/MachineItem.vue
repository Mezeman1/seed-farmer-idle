...
<template>
    <div
        class="bg-amber-50 rounded-lg shadow-sm p-5 border border-amber-200 transition-all duration-200 hover:translate-y-[-2px]">
        <div class="flex justify-between items-start mb-4">
            <div>
                <h3 class="text-xl font-bold text-amber-900 flex items-center">
                    <span class="mr-2">⚙️</span>{{ machine.name }}
                    <span class="ml-2 bg-green-100 text-green-800 text-sm px-2 py-0.5 rounded-full">Level {{
                        machine.level }}</span>
                </h3>
                <p class="text-amber-800 mt-1">{{ machine.description }}</p>
                <p v-if="levelUpDescription" class="text-sm text-green-700 mt-1 font-medium">
                    {{ levelUpDescription }}
                </p>
            </div>
            ...
            <div v-if="!machine.unlocked" class="text-right">
                <p class="text-sm text-amber-800 mb-2">Unlock Cost: {{ formatNumber(machine.unlockCost || 0) }} seeds
                </p>
            ...
            <div v-else class="text-right">
                <p class="text-sm text-amber-800">Progress to Level {{ machine.level + 1 }}</p>
        ...
        <div v-if="machine.unlocked" class="w-full bg-amber-100 rounded-full h-3 mb-4 overflow-hidden">
            <div class="bg-gradient-to-r from-green-600 to-green-500 h-3 rounded-full transition-all duration-300"
                :style="{ width: `${getLevelProgress}%` }">
            </div>
        </div>

        <!-- Locked Status (for locked machines) -->
        <div v-else class="w-full bg-amber-100 rounded-full h-3 mb-4 flex items-center justify-center overflow-hidden">
            <div class="text-xs text-amber-800 font-medium">Locked</div>
        </div>

        <div v-if="machine.unlocked" class="mb-4">
            <div class="flex justify-between text-sm mb-1">
                <span class="text-green-700 font-medium">Available Points: {{ formatNumber(machine.points) }}</span>
                <span class="text-amber-800">
                    Auto-levels at {{ autoLevelProgressText.target }} {{ autoLevelProgressText.unit }}
                    <span class="text-green-700">({{ autoLevelProgressText.remaining }} more)</span>
                </span>
            </div>
        </div>
        ...
        <div v-if="machine.unlocked" class="mt-6">
            <h4 class="font-semibold text-lg mb-3 border-b border-amber-200 pb-2 text-amber-900">Upgrades</h4>
            ...
            <div class="space-y-4">
                <div v-for="upgrade in machine.upgrades" :key="upgrade.id"
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            <h5 class="font-medium text-amber-900 flex items-center">
                                {{ upgrade.name }}
                                <span class="ml-2 bg-amber-100 text-amber-800 text-xs px-2 py-0.5 rounded-full">Level {{
                                    upgrade.level }}</span>
                            </h5>
                            <p class="text-sm text-amber-800 mt-1">{{ upgrade.description }}</p>

                            <!-- Unlock condition -->
                            <p v-if="!isUpgradeUnlocked(upgrade.id) && upgrade.unlockCondition"
                                class="text-sm text-orange-600 font-medium mt-2 flex items-center">
                                <span class="mr-1">🔒</span> {{ upgrade.unlockCondition.description }}
                            </p>

                            <p v-else class="text-sm text-green-700 font-medium mt-2">
                                Current effect: {{ upgrade.getEffectDisplay(upgrade.level, { machine }) }}
                            </p>

                            <!-- Detailed effects -->
                            <div v-if="upgrade.level > 0 && upgrade.effects.length > 1"
                                class="mt-2 bg-amber-100/50 p-2 rounded-md">
                                <p v-for="(effect, index) in getDetailedEffects(upgrade)" :key="index"
                                    class="text-xs text-amber-800 ml-2 flex items-start">
                        ...
                        <HoldButton @click="() => purchaseUpgrade(machine.id, upgrade.id)"
                            ...
                            size="sm"
                            class="bg-amber-200 hover:bg-amber-300 text-amber-900 disabled:bg-gray-200 disabled:text-gray-500 transition-colors">
                            <span class="flex items-center">
